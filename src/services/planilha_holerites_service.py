"""
Service para leitura e processamento da Planilha de Holerites
Responsabilidades:
- Carregar planilha Excel de holerites
- Parsear linhas com configurações de envio
- Montar diretórios de holerites
- Validar dados

Similar ao PlanilhaContatosService mas com colunas específicas para holerites.
"""
...
try:
    ...
    logger.warning("Pandas não instalado - PlanilhaHoleritesService não funcionará")


class TipoEnvioHolerite(str, Enum):
    """Modos de envio de holerites"""
    PLANILHA = "planilha"       # Enviar para contatos/grupos da planilha
    MONGODB = "mongodb"         # Enviar direto para funcionário via dados do MongoDB


class PlanilhaHoleritesService:
    r"""
    Lê e processa a Planilha de Holerites para envio
    
    Colunas esperadas na planilha:
    - ID: Identificador único
    - NOME COMPLETO: Nome do contato ou grupo
    ...
    - MÊS REFERÊNCIA: Mês do holerite (opcional, pode ser passado por parâmetro)
    - ANO REFERÊNCIA: Ano do holerite (opcional, pode ser passado por parâmetro)
    - DIRETÓRIO GERAL: Caminho base onde estão os PDFs de holerites
    - DIRETÓRIO ESPECÍFICO: Subpasta específica (opcional)
    
    Diferenças em relação à planilha de Folhas de Ponto:
    - Colunas LOCAL - CONTRATO - POLO são opcionais
    - PDFs são holerites individuais (não folhas de ponto)
    - Suporte a envio direto para funcionário (via MongoDB)
    """
    ...
    COLUNAS_OPCIONAIS = [
        ...
        "LOCAL - CONTRATO - POLO"  # Identificação para o assunto do email
    ]
    ...
    def _obter_caminho_padrao(self) -> str:
        ...
        # Tentar do .env
        caminho = os.getenv("PLANILHA_HOLERITES_PATH")
    ...
    def carregar(self, sheet_name: str | int = 0) -> bool:
        """
        Carrega a planilha em memória
        
        Args:
            sheet_name: Nome ou índice da aba (default: primeira aba (0))
        ...
        try:
            ...
            logger.info(f"✓ Planilha de Holerites carregada: {len(self._df)} linhas de '{self._planilha_path}'")
            return True
        
        except Exception as e:
            logger.error(f"Erro ao carregar planilha de holerites: {e}")
            return False
    ...
    def montar_diretorio_completo(
        ...
        r"""
        Monta o caminho completo do diretório dos holerites
        
        Padrão (se mês/ano fornecidos): {DIRETÓRIO GERAL}\{ANO}\{MÊS:02d}.{ANO}\{DIRETÓRIO ESPECÍFICO}
        Sem mês/ano: {DIRETÓRIO GERAL}\{DIRETÓRIO ESPECÍFICO}
        
        Args:
            diretorio_geral: Caminho base
            diretorio_especifico: Subpasta específica (opcional)
        ...
        """
        base = Path(diretorio_geral.strip())
        
        if mes and ano:
            caminho = base / str(ano) / f"{mes:02d}.{ano}"
        else:
            caminho = base
        
        if diretorio_especifico:
            caminho = caminho / diretorio_especifico.strip()
        
        return str(caminho)

    def listar_arquivos_pdf(self, diretorio: str, prefixo: str = "Recibo de Pagamento") -> List[str]:
        """
        Lista arquivos PDF em um diretório e subdiretórios (busca recursiva)
        Filtra apenas arquivos que começam com o prefixo especificado.
        
        Args:
            diretorio: Caminho do diretório raiz
            prefixo: Prefixo do nome do arquivo (default: 'Recibo de Pagamento')
        ...
        try:
            ...
            # Filtra apenas arquivos que começam com o prefixo
            arquivos = [
            ...
            if arquivos:    
                logger.debug(f"Encontrados {len(arquivos)} PDFs de holerites em {diretorio} (prefixo: '{prefixo}')")
            else:
                logger.debug(f"Nenhum PDF com prefixo '{prefixo}' encontrado em {diretorio}")
    ...
    def iterar_contatos(
        ...
        """
        Itera sobre os contatos da planilha de holerites
        
        Args:
            mes: Mês de referência (se None, tenta usar coluna MÊS REFERÊNCIA)
            ano: Ano de referência (se None, tenta usar coluna ANO REFERÊNCIA)
        ...
        for idx, row in self._df.iterrows():
            try:
                ...
                if not mes_ref:
                    try:
                        mes_ref = int(row.get("MÊS REFERÊNCIA", 0))
                    except (ValueError, TypeError):
                        mes_ref = None
                
                if not ano_ref:
                    try:
                        ano_ref = int(row.get("ANO REFERÊNCIA", 0))
                    except (ValueError, TypeError):
                        ano_ref = None
                ...
                yield {
                    ...
                    "local": row.get("LOCAL - CONTRATO - POLO", "").strip(),  # Para assunto do email
                    ...
                    "tipo_envio": TipoEnvioHolerite.PLANILHA.value
                }
    ...
    def listar_resumo(self) -> List[Dict[str, Any]]:
        ...
        for idx, row in self._df.iterrows():
            resumo.append({
                ...
                "nome": row.get("NOME COMPLETO", "").strip(),
                "empresa": row.get("EMPRESA", "").strip(),
                "diretorio": row.get("DIRETÓRIO GERAL", "").strip()[:50] + "...",
    ...
    def validar_planilha(self) -> Dict[str, Any]:
        ...
        for idx, row in self._df.iterrows():
            linha: int = idx + 2  # +2 por header e índice base 0
    ...
    def contar_pdfs_total(self, mes: int | None = None, ano: int | None = None) -> int:
        """
        Conta total de PDFs em todos os diretórios configurados
        ...
        Returns:
            Total de PDFs encontrados
        """
        total = 0
        for contato in self.iterar_contatos(mes, ano):
            total += len(contato.get("arquivos_pdf", []))
        return total


# Instância singleton
planilha_holerites_service = PlanilhaHoleritesService()
