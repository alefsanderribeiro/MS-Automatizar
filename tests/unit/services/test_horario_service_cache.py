"""
Unit tests for HorarioService cache-aside + invalidação por escrita.
"""
import os
import pytest
...
def _doc(descricao="08h-17h"):
    return {"_id": ObjectId(), "descricao": descricao, "nome_normalizado": cache_keys.normalizar_nome(descricao), "status": "ativo", "versao": 1}


def _make_service(collection, cache):
    with patch('src.services.horario_service.MONGODB_DISPONIVEL', True), \
         patch.dict(os.environ, {'CACHE_TTL': '300'}), \
         patch('src.services.horario_service.cache_service', cache), \
         patch('src.services.horario_service.dotenv.get_key') as md, \
         patch('src.services.horario_service.MongoDBConnectionPool') as mpc:
        ...
        svc = HorarioService(collection_name="horarios")
        svc._cache_horarios = {}
        return svc


class TestHorarioCacheAside:
    def test_buscar_por_id_cache_miss_popula_redis(self):
        ...
        assert cache_keys.horario_key(str(doc["_id"])) in cache.store
    ...
    def test_buscar_por_id_redis_hit_evita_mongo(self):
        ...
        collection = MagicMock()
        collection.find.return_value = []
        cache.store[cache_keys.horario_key(str(doc["_id"]))] = doc
...
class TestHorarioInvalidacaoAntiStale:
    def test_atualizar_campos_invalida_e_relê_do_banco(self):
        cache = _FakeCache()
        hid = str(ObjectId())
        old = {"_id": ObjectId(hid), "descricao": "A", "status": "ativo"}
        ...
        svc = _make_service(collection, cache)
        svc.buscar_por_id(hid)

        collection.find_one.return_value = new
        assert svc.atualizar_campos(hid, {"status": "inativo"}) is True
        assert cache_keys.horario_key(hid) not in cache.store
        assert svc.buscar_por_id(hid)["status"] == "inativo"

    def test_invalidar_cache_direcionada_remove_so_a_chave(self):
        cache = _FakeCache()
        a, b = str(ObjectId()), str(ObjectId())
        cache.store[cache_keys.horario_key(a)] = _doc()
        cache.store[cache_keys.horario_key(b)] = _doc()

        svc = _make_service(MagicMock(), cache)
        svc._invalidar_cache(registro_id=a)
        assert cache_keys.horario_key(a) not in cache.store
        assert cache_keys.horario_key(b) in cache.store
